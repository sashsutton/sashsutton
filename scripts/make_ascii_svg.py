...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "sasha-ascii.svg")
...
BRIGHTNESS = 1.0
GAMMA = float(os.environ.get("GAMMA", 1.00))   # >1 brightens mids -> sparser chars
SHARPEN = False
# luminance above this is forced to blank (space); raise it to keep more of an
# evenly-lit face in the ramp instead of washing it out to empty
WHITE_FLOOR = float(os.environ.get("WHITE_FLOOR", 0.90))
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">sasha@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'sasha@github:~$ whoami <tspan fill="{INK}">Sasha Sutton</tspan></text>')
