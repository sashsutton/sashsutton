...
ROWS = [
    ("host",),
    ("kv", "Now", "MSc DKAI @ Université Paris-Saclay"),
    ("kv", "Prev", "Research Intern @ LIS, Luminy"),
    ("kv", "Edu", "BSc Maths & CS, Aix-Marseille"),
    ("kv", "Also", "Music production, London · Fabric"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, C, TypeScript, Java, SQL"),
    ("kv", "ML / NLP", "PyTorch, BERTopic, CamemBERT, LDA"),
    ("kv", "Data", "NumPy, Pandas, Trafilatura"),
    ("kv", "Backend", "FastAPI, Next.js, MongoDB, AWS"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Paper on backpropagation · hal.science"),
    ("bul", "Headed for research in NLP + vision"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">sasha@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">sasha</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{KEY_X+114}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
